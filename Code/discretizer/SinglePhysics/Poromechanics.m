...
%% Post Processing - Valeriano
     if obj.elements.mesh.cellVTKType(1)== 10
         if obj.mesh.nCells==48
            elem_x_OK=find(round(obj.elements.cellCentroid(:,1),3)==0.1250);
            elem_y_OK=find(round(obj.elements.cellCentroid(:,2),3)==0.1250);
         elseif obj.mesh.nCells==384
            elem_x_OK=find(obj.elements.cellCentroid(:,1)==0.0625);
            elem_y_OK=find(obj.elements.cellCentroid(:,2)==0.0625);
         elseif obj.mesh.nCells==1296
            elem_x_OK=find(round(obj.elements.cellCentroid(:,1),4)==0.0417);
            elem_y_OK=find(round(obj.elements.cellCentroid(:,2),4)==0.0417);
         end
     elseif obj.elements.mesh.cellVTKType(1)== 12
         if obj.mesh.nCells==64
             elem_x_OK=find(round(obj.elements.cellCentroid(:,1),3)==0.1250);
             elem_y_OK=find(round(obj.elements.cellCentroid(:,2),3)==0.1250);
         end
     end
         elem_axis=intersect(elem_y_OK,elem_x_OK);
         highest_elem=intersect(find(obj.elements.cellCentroid(:,3)==max(obj.elements.cellCentroid(elem_axis,3))),elem_axis);
         Z_coords=obj.elements.cellCentroid(elem_axis,3);
         [sortZ_coords,id]=sort(Z_coords);
         % Sorting SigmaZ
         sortelem_axis=elem_axis(id);

         if isa(obj.material.db(1).ConstLaw, 'Elastic')
             fid1=fopen('Elastic_HN_sigma_z.txt','a');
             fprintf(fid1,'\n%.6f',stress(highest_elem,3));
             fid2=fopen('Elastic_HN_eps_z.txt','a');
             fprintf(fid2,'\n%.6f',strain(highest_elem,3));
         else
             fid1=fopen('HN_sigma_z.txt','a');
             fprintf(fid1,'\n%.6f',stress(highest_elem,3));
             fid2=fopen('HN_eps_z.txt','a');
             fprintf(fid2,'\n%.6f',strain(highest_elem,3));
             fid3=fopen('axis_sigma_z.txt','a');
             fprintf(fid3,'\n%.6f',stress(sortelem_axis,3));
             p=1/3*sum(stress(highest_elem,1:3));
             uno=[1 1 1 0 0 0]';
             W=diag([1 1 1 2 2 2]);
             xi=stress(highest_elem,:)'-p*uno;
             WNorm_xi=sqrt(xi'*W*xi);
             q=sqrt(3/2)*WNorm_xi;
             fid4=fopen('HN_p.txt','a');
             fprintf(fid4,'\n%.6f',p);
             fid5=fopen('HN_q.txt','a');
             fprintf(fid5,'\n%.6f',q);
             fid6=fopen('axis_eps_z.txt','a');
             fprintf(fid6,'\n%.6f',strain(sortelem_axis,3));
